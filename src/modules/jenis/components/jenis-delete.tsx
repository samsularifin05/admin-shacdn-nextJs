...
interface JenisDeleteProps {
  jenis: Jenis;
  onSuccess?: () => void;
}

export function JenisDelete({ jenis: row, onSuccess }: JenisDeleteProps) {
      ...
      <div className="flex items-start gap-3 p-4 bg-destructive/10 text-destructive rounded-lg border border-destructive/20">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm opacity-90">
            Are you sure you want to delete <strong>{row.kodeJenis}</strong>? This action is permanent and cannot be undone.
          </p>
        </div>
      </div>
