...
interface ServerDataTableResponse {
  meta?: {
    total?: number;
    totalPages?: number;
  };
...
const ServerDataTableComponent = <TData, TValue>(
  {
    endpoint,
    columns,
    actions,
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const result = (await response.json()) as ServerDataTableResponse;

      // Extract data based on path (e.g. "users" or "data")
      const list = Array.isArray(result[dataPath])
        ? (result[dataPath] as TData[])
        : [];
      setData(list);
...
export const ServerDataTable = forwardRef(ServerDataTableComponent) as <
  TData,
  TValue,
>(
